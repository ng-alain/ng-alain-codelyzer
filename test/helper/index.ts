...
import { loadRules, convertRuleOptions } from './util';
...
function lint(ruleName: string, source: string | SourceFile, options: any): LintResult {
  ...
  if (typeof source === 'string') {
    linter.lint(`C:\\Users\\asdf\\Desktop\\work\\ng-alain\\src\\app\\routes\\file.ts`, source, configuration);
  } else {
    const rules = loadRules(convertRuleOptions(configuration.rules), linterOptions.rulesDirectory, false);
    const res = [].concat.apply([], rules.map(r => r.apply(source) as any)) as RuleFailure[];
